...
(function() {
  ...
  /**
   * Get the 1-based index of a code block among all code blocks on the page
   */
  function getCodeBlockIndex(codeBlock) {
    if (!codeBlock) return 1;
    try {
      var allBlocks = document.querySelectorAll(config.codeBlockSelector);
      for (var i = 0; i < allBlocks.length; i++) {
        if (allBlocks[i] === codeBlock) {
          return i + 1;
        }
      }
    } catch (e) {
