	...
	*/
	...
	function validateConfig() {
		...
		if (!/^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(config.axiomHost)) {
			if (config.debug) console.warn("[Axiom Do11y] Invalid edge deployment domain:", config.axiomHost);
			return false;
		}
	...
	function getLinkIndex(link, href) {
		try {
			const escapedHref = typeof CSS !== "undefined" && typeof CSS.escape === "function" ? CSS.escape(href) : href.replace(/["\\]/g, "\\$&");
	...
	*/
	...
	function observeHeadings() {
		...
		document.querySelectorAll("h2, h3").forEach((h, i) => {
			if (!h.getAttribute("data-do11y-section-id")) h.setAttribute("data-do11y-section-id", "section-" + i);
	...
	function setupTabSwitchTracking() {
		...
		document.addEventListener("click", (e) => {
			...
			if (config.tabContainerSelector) baseSel += ", " + config.tabContainerSelector + " button, " + config.tabContainerSelector + " a, " + config.tabContainerSelector + " label";
	...
	function setupTocClickTracking() {
		...
		document.addEventListener("click", (e) => {
			const link = e.target.closest("a");
			if (!link) return;
			const tocContainer = link.closest(config.tocSelector ?? ".table-of-contents, [class*=\"toc\"], [class*=\"outline\"], [class*=\"TableOfContents\"], [class*=\"page-outline\"]");
	...
	function setupFeedbackTracking() {
		...
		document.addEventListener("click", (e) => {
			...
			if (!button) return;
			if (!button.closest(config.feedbackSelector ?? "[class*=\"feedback\"], [class*=\"helpful\"], [class*=\"rating\"], [class*=\"was-this\"], [data-feedback]")) return;
